...
public class AudioProcessor : MonoBehaviour
{
    ...
    private long _lastT, _nowT, _diff, _entries, _sum;
    ...
    private readonly int _nBand = 12;
    // number of bands

    [SerializeField] public float gThresh = 10f;
    // sensitivity

    int _blipDelayLen = 16;
    int[] _blipDelay;
    ...
    [Header("Events")] public OnBeatEventHandler onBeat;
    ...
    void Start()
    {
        ...
        framePeriod = (float)bufferSize / (float)_samplingRate;

        //initialize record of previous spectrum
        spec = new float[_nBand];
        for (int i = 0; i < _nBand; ++i)
            spec[i] = 100.0f;

        auco = new Autoco(maxlag, decay, framePeriod, getBandWidth());

        _lastT = getCurrentTimeMillis();
    }

    public void tapTempo()
    {
        _nowT = getCurrentTimeMillis();
        _diff = _nowT - _lastT;
        _lastT = _nowT;
        _sum = _sum + _diff;
        _entries++;

        int average = (int)(_sum / _entries);
    ...
    void Update()
    {
        ...
        if (audioSource.isPlaying)
        {
            ...
            {
                float specVal =
                    (float)System.Math.Max(-100.0f,
                        20.0f * (float)System.Math.Log10(averages[i]) + 160); // dB value of this band
                specVal *= 0.025f;
            ...
            if (smaxix == now)
            {
                ...
                {
                    timings.Add((float)AudioSettings.dspTime);
    ...
    {
        // var m = JsonUtility.ToJson(timings);
        // Debug.Log(m);
        // Debug.Log(timings.Count);
        File.WriteAllText(Path.Combine(Application.dataPath, "Resources/timings.json"), JsonUtility.ToJson(
            new TimingsList
            {
                Timings = timings
            }));
    ...
    {
        return (2f / (float)bufferSize) * (_samplingRate / 2f);
    }
    ...
    public int freqToIndex(int freq)
    {
        ...
        if (freq > _samplingRate / 2 - getBandWidth() / 2)
            return (bufferSize / 2);
        // all other cases
        float fraction = (float)freq / (float)_samplingRate;
    ...
    public void computeAverages(float[] data)
    {
        for (int i = 0; i < 12; i++)
        {
            ...
            if (i == 0)
                ...
                lowFreq = (int)((_samplingRate / 2) / (float)System.Math.Pow(2, 12 - i));
            int hiFreq = (int)((_samplingRate / 2) / (float)System.Math.Pow(2, 11 - i));
        ...
        public Autoco(int len, float alpha, float framePeriod, float bandwidth)
        {
            ...
            for (int i = 0; i < del_length; ++i)
            {
                ...
                rweight[i] = (float)System.Math.Exp(-0.5f *
                                                    System.Math.Pow(
                                                        System.Math.Log(bpms[i] / wmidbpm) / System.Math.Log(2.0f) /
                                                        woctavewidth, 2.0f));
            }
        }
